...
export function ArticlesTable({ articles, onDeleteArticle, onEditArticle }: any) {
  ...
  const getCategoryBadge = (cat: string) => {
    ...
    const config: any = { news: 'bg-red-50 text-[#c91218] border-red-200', tutorial: 'bg-green-100 text-green-700 border-green-200', sharing: 'bg-purple-100 text-purple-700 border-purple-200' }
      ...
      <Table>
        ...
        <TableBody>
          {articles.map((article: any) => (
            <TableRow key={article.id} className="hover:bg-red-50/30 border-slate-50 transition-all duration-500 group">
              <TableCell className="py-8 pl-10">
                <div className="flex items-start gap-5">
                  ...
                  <div className="flex flex-col justify-center">
                    <h3 className="font-black text-slate-800 text-lg leading-tight line-clamp-1 mb-1 group-hover:text-[#ed1c24] transition-colors">{article.title}</h3>
              ...
              <TableCell className="text-right pr-10">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-56 p-2 rounded-[1.5rem] shadow-2xl border-none">
                    <DropdownMenuItem className="rounded-xl py-3 cursor-pointer gap-3 font-bold text-slate-600" onClick={() => onEditArticle(article)}><Edit3 className="h-4 w-4 text-[#ed1c24]" /> Chỉnh sửa chuyên sâu</DropdownMenuItem>
