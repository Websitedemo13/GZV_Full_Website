      ...
      <Tabs defaultValue="notifications" className="space-y-6">
        ...
        <TabsContent value="notifications" className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <SettingSwitch icon={<Mail className="h-5 w-5 text-[#ed1c24]" />} title="Email thông báo" desc="Nhận email khi có tin nhắn liên hệ hoặc cập nhật quan trọng." checked={settings.emailNotifications} onChange={(v) => handleSettingChange("emailNotifications", v)} />
        ...
        <TabsContent value="privacy" className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <SettingSwitch icon={<Globe className="h-5 w-5 text-[#ed1c24]" />} title="Hồ sơ công khai" desc="Cho phép hiển thị hồ sơ trong các khu vực nội bộ phù hợp." checked={settings.publicProfile} onChange={(v) => handleSettingChange("publicProfile", v)} />
        ...
        <TabsContent value="appearance" className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-6">
              <div><Label className="text-base font-medium mb-3 block">Chủ đề</Label><div className="grid grid-cols-3 gap-3">{[{ id: "light", label: "Sáng", icon: "☀️" }, { id: "dark", label: "Tối", icon: "🌙" }, { id: "system", label: "Hệ thống", icon: "⚙️" }].map((option) => <button key={option.id} onClick={() => setTheme(option.id)} className={`p-4 rounded-lg border-2 transition-all ${theme === option.id ? "border-[#ed1c24] bg-red-50 dark:bg-red-950/20" : "border-gray-200 dark:border-gray-700 bg-white/70 dark:bg-gray-700/30"}`}><div className="text-2xl mb-2">{option.icon}</div><p className="font-medium text-sm">{option.label}</p></button>)}</div></div>
              <div><Label className="text-base font-medium mb-3 block">Cỡ chữ</Label><div className="grid grid-cols-3 gap-3">{[{ id: "small", label: "Nhỏ" }, { id: "normal", label: "Chuẩn" }, { id: "large", label: "Lớn" }].map((option) => <button key={option.id} onClick={() => handleSettingChange("fontSize", option.id as AdminSettings["fontSize"])} className={`p-4 rounded-lg border-2 transition-all ${settings.fontSize === option.id ? "border-[#ed1c24] bg-red-50 dark:bg-red-950/20" : "border-gray-200 dark:border-gray-700 bg-white/70 dark:bg-gray-700/30"}`}><p className="font-medium text-sm">{option.label}</p></button>)}</div></div>
            </CardContent>
          </Card>
        </TabsContent>
