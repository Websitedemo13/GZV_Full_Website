...
const BlogCard = ({ id, title, excerpt, image, authors, publishDate, category, slug, readTime }: BlogCardProps) => {
  ...
  return (
    <motion.div
      ...
      className="h-full"
    >
      <Card className="h-full flex flex-col bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border border-slate-200 dark:border-slate-800 shadow-[0_14px_34px_rgba(15,23,42,0.08)] hover:shadow-[0_24px_48px_rgba(8,47,87,0.14)] transition-all duration-300 overflow-hidden group rounded-none">
            ...
            <div className="flex items-center gap-2">
              <div className="flex -space-x-3">
                {displayAuthors.map((auth, i) => (
                  <Avatar key={i} className="h-8 w-8 rounded-md border-2 border-white dark:border-slate-900 shadow-sm transition-transform hover:-translate-y-1">
        ...
        <CardFooter className="px-6 pb-6 pt-0">
          <Link href={`/tin-tuc/${slug}`} className="w-full">
            <Button className="w-full btn-primary rounded-none h-11 text-sm font-semibold">
              Đọc bài viết
